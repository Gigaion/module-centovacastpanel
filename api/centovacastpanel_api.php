...
class CentovacastpanelApi
{
    ...
    public function getPackagesList()
    {
        ...
        $response = $this->apiRequest('packs', array());
    ...
    public function createRadio(array $params)
    {
		...
		*/
		...
		$parameters = array(
			...
			'sourcepassword' => $params['radiopassword'].'DJ',
		...
		if (isset($params['servertype'])) {
			$parameters['servertype'] = $params['servertype'];
		}
    ...
    public function changePackage($radiousername, $configoptions)
    {
		...
		//[Limits] Tab
		if($configoptions['maxbitrate'] > 0) {
			$parameters['maxbitrate'] = floor($parameters['maxbitrate']);
		}
		if($configoptions['diskquota'] > 0) {
			$parameters['diskquota'] = floor($configoptions['diskquota']);
		}
		if($configoptions['transferlimit'] > 0) {
			$parameters['transferlimit'] = floor($configoptions['transferlimit']);
		}
