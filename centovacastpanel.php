...
/**
 * Centovacastpanel Module.
 *
 * @copyright Gigaion LLC
 * @license MIT License
 * @see license.md (MIT License)
 */
class Centovacastpanel extends Module
{
    /**
    ...
    public function __construct()
    {
        ...
        Language::loadLang('centovacastpanel', null, dirname(__FILE__) . DS . 'language' . DS);
		
		
		$this->debug = false;
		$this->logfile('RUNNING Centovacastpanel construct: started module...');
    }
    ...
    {
        return [
            'tabStats' => Language::_('Centovacastpanel.tab_stats', true)
        ];
    }
    ...
    {
        return [
            'tabClientActions' => Language::_('Centovacastpanel.tab_client_actions', true),
            'tabClientStats' => Language::_('Centovacastpanel.tab_stats', true)
        ];
    }
    ...
    {
        return ['first' => Language::_('Centovacastpanel.order_options.first', true)];
    }
    ...
    public function getPackageFields($vars = null)
    {
		...
		/*
        $package = $fields->label(Language::_('Centovacastpanel.package_fields.package', true), 'package');
        $package->attach(
            $fields->fieldSelect(
                'meta[package]',
    ...
    public function manageModule($module, array &$vars)
    {
        ...
        $this->view->setDefaultView('components' . DS . 'modules' . DS . 'centovacastpanel' . DS);
    ...
    public function getAdminAddFields($package, $vars = null)
    {
		...
		$radiousername = $fields->label(Language::_('Centovacastpanel.service_field.radiousername', true), 'radiousername');
		...
		$radiopassword = $fields->label(Language::_('Centovacastpanel.service_field.radiopassword', true), 'radiopassword');
    ...
    public function getAdminEditFields($package, $vars = null)
    {
        ...
        $radiousername->attach($fields->tooltip(Language::_('Centovacastpanel.service_field.radiousername.tooltip', true)));
        $fields->setField($radiousername);

        $radiopassword = $fields->label(
            Language::_('Centovacastpanel.service_field.radiopassword', true),
            'radiopassword'
        );
        $radiopassword->attach($fields->fieldPassword('radiopassword', ['id' => 'radiopassword']));
        $radiopassword->attach($fields->tooltip(Language::_('Centovacastpanel.service_field.radiopassword.tooltip', true)));
        $fields->setField($radiopassword);
        ...
        $rules = [
            'radiousername' => [
                'empty' => [
                    ...
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Centovacastpanel.!error.radiousername.empty', true)
                ]
            ],
        ];
    ...
    public function addService(
        ...
        if (!$row) {
            $this->Input->setErrors(
                ['module_row' => ['missing' => Language::_('Centovacastpanel.!error.module_row.missing', true)]]
            );
		...
		$params['diskquota'] = $vars['configoptions']['diskquota'];
		
		
		$params['servertype'] = $vars['configoptions']['servertype'];
		if($params['servertype'] != '') {
			//Set to correct uppercase notation (incase its all lowercase and etc)
			if(stristr($params['servertype'], 'IceCast')) {
				$params['servertype'] = 'IceCast';
			}
			elseif(stristr($params['servertype'], 'ShoutCast2')) {
				$params['servertype'] = 'ShoutCast2';
			}			
			elseif(stristr($params['servertype'], 'ShoutCast')) {
				$params['servertype'] = 'ShoutCast';
			}
		}
    ...
    public function editService($package, $service, array $vars = null, $parent_package = null, $parent_service = null)
    {
		...
		if (($row = $this->getModuleRow())) {
			...
			if(!empty($vars['configoptions'])) {
				...
				$response = $this->parseResponse($api->changePackage($service_fields->radiousername, $vars['configoptions']));
    ...
    public function suspendService($package, $service, $parent_package = null, $parent_service = null)
    {
        if (($row = $this->getModuleRow())) {
            ...
            if (!$response['status']) {
                $this->Input->setErrors(['api' => ['error' => '[Suspend] '.Language::_('Centovacastpanel.!error.api.internal', true)]]);
            }
        }
    ...
    public function unsuspendService($package, $service, $parent_package = null, $parent_service = null)
    {
        if (($row = $this->getModuleRow())) {
            ...
            // If the action fails then set an error
            if (!$response['status']) {
                $this->Input->setErrors(['api' => ['error' => '[Unsuspend] '.Language::_('Centovacastpanel.!error.api.internal', true)]]);
            }
        }
    ...
    public function cancelService($package, $service, $parent_package = null, $parent_service = null)
    {
        if (($row = $this->getModuleRow())) {
            ...
            // If the action fails then set an error
            if (!$response['status']) {
                $this->Input->setErrors(['api' => ['error' => '[Cancel] '.Language::_('Centovacastpanel.!error.api.internal', true)]]);
            }
        }
    ...
    private function parseResponse($response)
    {
        ...
        if ($response['status'] == false) {
            $this->Input->setErrors(['api' => ['error' => '[Parse] '.Language::_('Centovacastpanel.!error.api.internal', true)]]);
            $success = false;
        }
     ...
     *
     * @param string $password The centovacastpanel password
     * @param string $ipaddress The ip address of the server
     * @param bool $usessl Whether to use https or http
     * @return CentovacastpanelApi the CentovacastpanelApi instance, or false if the loader fails to load the file
     */
    private function getApi($adminapikey, $ipaddress, $usessl, $adminusername, $useproxy)
    {
        Loader::load(dirname(__FILE__) . DS . 'api' . DS . 'centovacastpanel_api.php');

        return new CentovacastpanelApi($adminapikey, $ipaddress, $usessl, $adminusername, $useproxy);
    }
        ...
        $rules = [
		    'server_name' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Centovacastpanel.!error.server_name_valid', true)
                ]
            ],
			'ipaddress' => [
                'valid' => [
                    'rule' => [[$this, 'validateIP']],
                    'message' => Language::_('Centovacastpanel.!error.ipaddress_valid', true)
                ]
            ],
			'hostname' => [
                'valid' => [
                    'rule' => [[$this, 'validateHostName']],
                    'message' => Language::_('Centovacastpanel.!error.hostname_valid', true)
                ]
            ],
            'adminusername' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Centovacastpanel.!error.adminusername_valid', true)
                ]
            ],
            'adminapikey' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Centovacastpanel.!error.adminapikey_valid', true)
                ],
                'valid_connection' => [
                    'rule' => [
                    ...
                    'message' => '[Validate Connection] '.Language::_('Centovacastpanel.!error.api.internal', true)
                ]
            ]
        ];
        ...
        $rules = [
			...
			'meta[package]' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Centovacastpanel.!error.meta[package].valid', true)
                ]
            ],
			*/
        ];
    ...
    public function validateConnection($adminapikey, $servernamelabel, $ipaddress, $hostname, $adminusername, $usessl, $useproxy)
    {
		try {
            ...
            $result = $api->validateConnectionAPI(['_#$%^3456#$%^#456[{g']);
            // Log the response
